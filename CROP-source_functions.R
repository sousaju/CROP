...
mltpl_remove=function (data,det,name="",ccth,rtw,mcs,rt,maxrtw=NULL,funit,rtunit){
  ...
  write.table(data_mod2,paste(name,"_CROPped_ccth_",ccth,"_rtw+-",rtw,"_data_without_multiplicities.csv",sep=""),sep=";")  
  
  otp_table=t(data_mod[-c(1,2),])
  otp_table=cbind(rownames(otp_table),otp_table)
  colnames(otp_table)[1]=funit
  rownames(otp_table)=NULL
  write.table(otp_table,paste(name,"_CROPped_ccth_",ccth,"_rtw+-",rtw,"_output_table.csv",sep=""),sep=";",row.names=FALSE) 
  ...
  write.table(ls_clust,paste(name,"_CROPped_ccth_",ccth,"_rtw+-",rtw,"_list_of_clusters.csv",sep=""),sep=";")
...
CROP=function(data, name="", ccth = 0.75, rtw = 0.02, mcs=100, maxrtw = NULL, rtunit="min", funit="MW") {
  ...
  if (class(maxrtw)!="NULL") {
    ...
    cairo_pdf(paste(name=name,"_CROPped_ccth_",ccth,"_rtw+-",rtw,"_correlation_network.pdf",sep=""),w=25,h=25)
